...
int distSimpleDPcomputation() {
  ...
  */    
	...
	if(S[i] == T[j])  D[i][j] = D[i-1][j-1];
          ...
          D[i][j] = D[i-1][j-1] + 1; // ipotesi modifica carattere
	  if( D[i-1][j] + 1 < D[i][j]) // ipotesi del car in S
	    D[i][j] = D[i-1][j] + 1;
	  if( D[i][j-1] + 1 < D[i][j]) // ipotesi del car in T (ins in S)
	    D[i][j] = D[i][j-1] + 1;
	}
    }  
    /* fprintf(stderr, "   -");
